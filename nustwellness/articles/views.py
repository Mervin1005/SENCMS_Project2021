...
def article_detail(request,word):
  ...
  return render(request, 'front/article_detail.html', {'site':site, 'articles':articles, 'categories':categories, 'subcategories':subcategories, 'lastarticles':lastarticles, 'showarticles':showarticles, 'poparticles':poparticles, 'poparticles2':poparticles2, 'tag':tag, 'trending':trending})
    ...
    try:
      ...
      if str(myfile.content_type).startswith("image"):
        ...
        if myfile.size < 5000000 :
          ...
          b = Articles(name=articletitle, short_txt=articletextshort, body_text=articletext, date=today, picname=filename, picurl=url, writer="-", catname=articlename, catid=articleid, show = 0, time=time, ocatid=ocatid, tag=tag)
          b.save()
  ...
  if request.method == 'POST':
    ...
    try:
      ...
      if str(myfile.content_type).startswith("image"):
        ...
        b = Articles.objects.get(pk=pk)         #fss = FileSystemStorage()fss.delete(b.picname)
...
def all_articles(request):
  ...
  return render(request, 'front/all_articles2.html', {'site':site, 'categories':categories, 'subcategories':subcategories, 'lastarticles':lastarticles, 'poparticles': poparticles, 'poparticles2':poparticles2, 'trending':trending, 'serviceproviders':serviceproviders, 'allarticles':allarticles})
...
def all_articles_search(request):
  ...
  #allarticles = Articles.objects.all
  #allarticles = Articles.objects.filter(name=txt)
